...
def search_books(query):
    ...
    if response.status_code == 200:
        ...
        for item in response_body["items"]:
            ...
            thumbnail_link = image_links.get('thumbnail') if image_links else None                 
            books.append(Book(title=title, google_books_id=google_books_id, 
                authors=author_string, thumbnail_link=thumbnail_link))
        return books
    else:
        raise requests.ConnectionError(response.status_code) # todo what exception should be raised here?
